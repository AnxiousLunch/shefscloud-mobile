...
const ChefDetailScreen = () => {
  ...
  const { chefId } = route.params as { chefId: string };
  const { authToken } = useSelector((state: RootState) => state.user);
  const [chefAndDishes, setChefAndDishes] = useState<any>(null);
...
// ChefContent.tsx
const ChefContent = ({ chefAndDishes }: { chefAndDishes: any }) => {
  return (
...
// FilterAndDate.tsx
const FilterAndDate = ({ chefAndDishes }: { chefAndDishes: any }) => {
  ...
  useEffect(() => {
    ...
    const filteredDishes = chefAndDishes?.menus
      ?.filter((menu: any) => menu.is_live === 1)
      ?.filter((dish: any) => {
        ...
        const slotMatch = !sortingWithSlot || dish.availability_time_slots?.some(
          (slot: any) => sortingWithSlot === slot.availability_time_slots_id
        );
...
// Helper function
const calculateTotalPrice = (dish: any) => {
