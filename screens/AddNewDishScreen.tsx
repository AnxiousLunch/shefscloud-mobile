...
//   const authToken = useSelector((state: RootState) => state.user.userInfo?.access_token);
//   const params = useLocalSearchParams();
//   const router = useRouter();
  
//   // Parse dishToEdit with proper typing
//   const [dishToEdit, setDishToEdit] = useState<ChefMenuState | null>(null);
  
//   useEffect(() => {
//     if (params.dishToEdit) {
//       try {
//         const parsed = JSON.parse(params.dishToEdit as string);
...
//   const [chefMenu, setChefMenu] = useState<ChefMenuState>(chefMenuInitialState);
...
//   const [heatInstruction, setHeatInstruction] = useState<HeatInstruction[]>([]);
//   const [ingredientOptions, setIngredientOptions] = useState<IngredientOption[]>([]);
//   const [timeSlot, setTimeSlot] = useState<TimeSlot[]>([]);
  
//   // Selected values
//   const [cusineSelectedOptions, setCusineSelectedOptions] = useState<TagOption[]>([]);
//   const [selectedAvailabilitySlot, setSelectedAvailabilitySlot] = useState<TimeSlot[]>([]);
...
//         const selected = formattedTimeSlots.filter((slot: TimeSlot) =>
...
//           const temp = formatedTags.filter((item: TagOption) =>
...
//           const ingredientsArray = formatedIngredients.filter((item: IngredientOption) =>
...
//   const updateFields = (fields: Partial<ChefMenuState>) => {
...
//       Alert.alert('Warning', 'Please select an image for your dish');
//       return;
//     }
...
//     // 2. Add days of week
//     const days = ['is_monday', 'is_tuesday', 'is_wednesday', 'is_thursday', 'is_friday', 'is_saturday', 'is_sunday'];
//     days.forEach(day => {
...
//       packaging: '1', // Use correct packaging ID from your backend
//       user_id: '74' // From your logs
//     };
...
const AddNewDishScreen = () => {
  ...
  const onSubmit = async () => {
    try {
      ...
      days.forEach(day => {
        formData.append(day, chefMenu[day as keyof ChefMenuState] ? '1' : '0');
      });
